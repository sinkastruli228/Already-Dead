...
namespace AlreadyDead
{
    ...
    public sealed class MagicStaff : MonoBehaviour
    {
        ...
        public string DisplayName => element == MagicElement.Fire ? "ОГНЕННЫЙ ПОСОХ" :
            element == MagicElement.Frost ? "МОРОЗНЫЙ ПОСОХ" : "ЭЛЕКТРИЧЕСКИЙ ПОСОХ";
        ...
        public bool TryCast(Vector2 aimDirection, AimCamera camera)
        {
            ...
            float interval = element == MagicElement.Fire ? tuning.fireCastInterval :
                element == MagicElement.Frost ? tuning.frostCastInterval : tuning.lightningCastInterval;
            ...
            if (blocked)
                MagicProjectile.Impact(blocked.point, element, primitiveSprite, primitiveMaterial);
            else
                MagicProjectile.Spawn(castPoint, direction, element, tuning, primitiveSprite, primitiveMaterial);

            CastsMade++;
            LastCastDirection = direction;
            if (camera != null) camera.Kick(0.055f, 0.09f);
            return true;
        }
    }
}
